...
@testset "Segmentation-A" begin
    ...
    ice_water_discriminated_image = float64.(load("$(test_data_dir)/matlab_ice_water_discrim.png"))
    ...
    @time segmented_ice_cloudmasked = IceFloeTracker.segmented_ice_cloudmasking(ice_water_discriminated_image, cloudmask, ice_labels)
    ...
    @test test_similarity(convert(BitMatrix, IceFloeTracker.apply_landmask(segmented_ice_cloudmasked, landmask)), matlab_segmented_ice_cloudmasked, 0.051)
