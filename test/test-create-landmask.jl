@testset "Create Landmask" begin
    ...
    matlab_landmask_no_dilate = float64.(load(matlab_landmask_no_dilate_file)[lm_test_region...])
    ...
    @test landmask_no_dilate = IceFloeTracker.create_landmask_no_dilate(lm_image)

    # @time masked_image = IceFloeTracker.apply_landmask(test_image, landmask)
    @time masked_image_no_dilate = IceFloeTracker.apply_landmask_no_dilate(test_image, landmask_no_dilate)
    ...
    @test test_similarity(.!landmask_no_dilate, convert(BitMatrix, matlab_landmask_no_dilate), 0.005)
