...
"""
function get_max_label(d::Dict{Int, Int})
    drev = Dict{Int,Int}()
    mx = 0
    for (k,v) in d
        drev[v] = k
...
"""
function filt_except_label(labeled_arr::Array{Int64, 2}, label::Int64)
    outlabel = similar(labeled_arr)
    for (i, v) in enumerate(labeled_arr)
        v != label ? outlabel[i] = 0 : outlabel[i] = label
    end
    return outlabel
end
...
function filt_except_label!(labeled_arr::Array{Int64, 2}, label::Int64)
    for (i, v) in enumerate(labeled_arr)
        v != label ? labeled_arr[i] = 0 : labeled_arr[i] = label
    end
    return labeled_arr
end
