...
function get_ice_labels(
    ...
    mask_ice_band_2 = @view(cv[2, :, :]) .> band_2_threshold #230 / 255
    mask_ice_band_1 = @view(cv[3, :, :]) .> band_1_threshold #240 / 255
    ice_labels = mask_ice_band_7 .* mask_ice_band_2 .* mask_ice_band_1
    ...
    if sum(ice_labels) == 0
        ...
        ## Second relaxation attempt
        if sum(ice_labels) == 0
            ref_image_band_2 = @view(cv[2, :, :])
            ref_image_band_1 = @view(cv[3, :, :])

            band_2_peak, band_1_peak =
                find_reflectance_peaks.(
                    [ref_image_band_2, ref_image_band_1];
                    possible_ice_threshold=possible_ice_threshold,
                )

            # check both peaks are not nothing
            if all(.!isnothing.([band_2_peak, band_1_peak]))
                mask_ice_band_2 = @view(cv[2, :, :]) .> band_2_peak / 255
                mask_ice_band_1 = @view(cv[3, :, :]) .> band_1_peak / 255
                ice_labels = mask_ice_band_7 .* mask_ice_band_2 .* mask_ice_band_1
            end
