@inline function Base.getproperty(ϕ::GridDeformation, name::Symbol)
    ...
    elseif name === :knots
        Base.depwarn(
            "the field name of `GridDeformation` has changed from `knots` to `nodes`.",
            :getproperty,
        )
        return getfield(ϕ, :nodes)
    end
    return error("field ", name, " is not available")
end

function GridDeformation(
    u::AbstractArray{FV,N}, sz::NTuple{N,L}
) where {FV<:SVector,N,L<:Integer}
    Base.depwarn(
        "`GridDeformation(u, size(fixed))` is deprecated, use `GridDeformation(u, axes(fixed))` instead.",
        :GridDeformation,
    )
...
function GridDeformation(u::AbstractArray{FV,0}, nodes::Tuple{}) where {FV<:SVector}
    return error("node tuple cannot be empty")
end

function GridDeformation(u, nodes::AbstractVector{<:Integer})
    Base.depwarn(
        "`GridDeformation(u, [size(fixed)...])` is deprecated, pass the axes of `fixed` instead.",
        :GridDeformation,
    )
...
@deprecate getindex(
    ϕ::GridDeformation{T,N,A}, xs::Vararg{Number,N}
) where {T,N,A<:AbstractInterpolation} ϕ(xs...)
