...
struct GridDeformation{T,N,A<:AbstractArray,L} <: AbstractDeformation{T,N}
    ...
    function GridDeformation{T,N,A,L}(
        u::AbstractArray{FV,N}, nodes::NTuple{N,L}
    ) where {T,N,A,L,FV<:SVector}
        typeof(u) == A || error("typeof(u) = $(typeof(u)), which is different from $A")
        length(FV) == N || throw(
            DimensionMismatch("Dimensionality $(length(FV)) must match $N node vectors")
        )
        for d in 1:N
            size(u, d) == length(nodes[d]) || error(
                "size(u) = $(size(u)), but the nodes specify a grid of size $(map(length, nodes))",
            )
        end
        return new{T,N,A,L}(u, nodes)
    end
    function GridDeformation{T,N,A,L}(
        u::ScaledInterpolation{FV,N}
    ) where {T,N,A,L,FV<:SVector}
...
function GridDeformation(
    ...
    T = eltype(FV)
    length(FV) == N ||
        throw(DimensionMismatch("$N-dimensional array requires SVector{$N,T}"))
    return GridDeformation{T,N,typeof(u),L}(u, nodes)
end

# With image axes
function GridDeformation(
    u::AbstractArray{FV,N}, axs::NTuple{N,L}
...
function GridDeformation(u::AbstractArray{T}, nodes::NTuple{N}) where {T<:Number,N}
    ndims(u) == N + 1 ||
        error("`u` needs $(N+1) dimensions for $N-dimensional deformations")
...
function GridDeformation(u, nodes::AbstractVector{V}) where {V<:AbstractVector}
...
function GridDeformation(u::ScaledInterpolation{FV}) where {FV<:SVector}
    N = length(FV)
    ndims(u) == N || throw(
        DimensionMismatch("Dimension $(ndims(u)) incompatible with vectors of length $N"),
    )
...
function griddeformations(u::AbstractArray{T}, nodes::NTuple{N}) where {N,T<:Number}
    ndims(u) == N + 2 ||
        error("Need $(N+2) dimensions for a vector of $N-dimensional deformations")
    size(u, 1) == N || error("First dimension of u must be of length $N")
...
function griddeformations(u::AbstractArray{FV}, nodes::NTuple{N}) where {N,FV<:SVector}
    ndims(u) == N + 1 ||
        error("Need $(N+1) dimensions for a vector of $N-dimensional deformations")
    length(FV) == N ||
        throw(DimensionMismatch("Dimensionality $(length(FV)) must match $N node vectors"))
...
function Base.copy(ϕ::GridDeformation{T,N,A,L}) where {T,N,A,L}
    return (u = copy(ϕ.u); GridDeformation{T,N,typeof(u),L}(u, map(copy, ϕ.nodes)))
end
...
function _extrapolate(A::AbstractArray, nodes, BC)
    return _extrapolate(interpolate(A, BSpline(Quadratic(BC))), nodes, BC)
end
function _extrapolate(A::AbstractInterpolation, nodes, BC)
    return _extrapolate(extrapolate(A, Line()), nodes, BC)
end
...
function Interpolations.interpolate(ϕ::InterpolatingDeformation, args...)
    return error("ϕ is already interpolating")
end
function Interpolations.interpolate!(ϕ::InterpolatingDeformation, args...)
    return error("ϕ is already interpolating")
end
...
function _extrapolate!(A, nodes, BC)
    return _extrapolate(interpolate!(A, BSpline(Quadratic(BC))), nodes, BC)
end
_extrapolate!(A::AbstractInterpolation, nodes, BC) = error("ϕ is already interpolating")

function vecindex(
    ϕ::GridDeformation{T,N,A}, x::SVector{N}
) where {T,N,A<:AbstractInterpolation}
...
function _similarϕ(ϕref::InterpolatingDeformation, udata)
    ...
    itp = Interpolations.BSplineInterpolation(
        floattype(eltype(udata)), udata, itpref.it, itpref.parentaxes
    )
...
@inline function (ϕ::GridDeformation{T,N,A})(
    xs::Vararg{Number,N}
) where {T,N,A<:AbstractInterpolation}
    return ϕ.u(xs...) .+ xs
end
...
function (ϕ::GridDeformation{T,N})(xs::Vararg{Number,N}) where {T,N}
    return error("call `ϕi = interpolate(ϕ)` and use `ϕi` for evaluating the deformation.")
end

@inline (ϕ::GridDeformation{T,N})(xs::SVector{N}) where {T,N} = ϕ(Tuple(xs)...)

# Composition ϕ_old(ϕ_new(x))
function (ϕ_old::GridDeformation{T1,N,A})(
    ϕ_new::GridDeformation{T2,N}
) where {T1,T2,N,A<:AbstractInterpolation}
    uold, nodes = ϕ_old.u, ϕ_old.nodes
    if !isa(ϕ_new.u, AbstractInterpolation)
        ϕ_new.nodes == nodes ||
            error("If nodes are incommensurate, ϕ_new must be interpolating")
    end
    ucomp = _compose(uold, ϕ_new.u, nodes)
    return GridDeformation(ucomp, nodes)
end

(ϕ_old::GridDeformation)(ϕ_new::GridDeformation) = error("ϕ_old must be interpolating")
...
function compose(
    ...
    return GridDeformation(ucomp, nodes), g
end
...
"""
function compose(
    ϕsi_old::AbstractVector{G1}, ϕs_new::AbstractVector{G2}
) where {G1<:GridDeformation,G2<:GridDeformation}
    n = length(ϕs_new)
    length(ϕsi_old) == n || throw(
        DimensionMismatch(
            "vectors-of-deformations must have the same length, got $(length(ϕsi_old)) and $n",
        ),
    )
...
function compose(f::Function, ϕ_new::GridDeformation{T,N}) where {T,N}
    ...
    return ϕ_new, fill(similar_type(SArray, T, Size(N, N))(1.0I), size(ϕ_new.u))
end
