function init(f, xsplits, lower::AbstractVector, upper::AbstractVector)
    # Validate the inputs
    n = length(lower)
    length(upper) == length(xsplits) == n || throw(
        DimensionMismatch("inconsistent dimensionality among lower, upper, and splits")
    )
    for i in 1:n
        xsi = xsplits[i]
        length(xsi) == 3 || throw(
            DimensionMismatch(
                "3 values along each dimension required, got $(xsi) along dimension $i"
            ),
        )
        lower[i] <= xsi[1] < xsi[2] < xsi[3] <= upper[i] || error(
            "splits must be in strictly increasing order and lie between lower and upper, got $xsi along dimension $i",
        )
...
@noinline function _init(f, xstar, xsplits, lower, upper)
    ...
    for i in 1:n
        ...
        xcur, fcur = box.parent.xvalues[box.parent_cindex],
...
function split!(
    box::Box{T}, f, xtmp, splitdim, xsplit, lower::Real, upper::Real, xcur, fcur
) where {T}
    ...
    if qcoef > 0
        ...
        if bb[1] <= xvert <= bb[2]
            ...
            add_children!(
                box,
                splitdim,
                MVector3{T}(xf1[1], xf2[1], xf3[1]),
                MVector3{T}(xf1[2], xf2[2], xf3[2]),
                lwr,
                upr,
            )
            trimschedule!(mes, box, splitdim, x0, lower, upper)
...
function trisect!(box::Box, f, xtmp, splitdim, p=find_parent_with_splitdim(box, splitdim))
    ...
    @assert(xcur == xtmp[splitdim])
    fcur = box.parent.fvalues[box.parent_cindex]
    return trisect!(box, f, xtmp, splitdim, bb, Δx, xcur, fcur)
end

# Returns target box, value, and α. Uses α=0 to signal failure.
function quasinewton!(
    box::Box{T}, B, g, c, scale, f::Function, x0, lower, upper, itermax=20
) where {T}
    x = position(box, x0)
    fbox = fmin = value(box)
...
function lls_bounded(
    H::AbstractMatrix{T}, f::VT, lower::V, upper::V, tol=sqrt(eps(T))
) where {T,VT<:AbstractVector{T},V<:AbstractVector}
...
function lls_bounded!(
    x::VT, H::AbstractMatrix{T}, f::VT, lower::V, upper::V, tol=sqrt(eps(T))
) where {T,VT<:AbstractVector{T},V<:AbstractVector}
    return lls_bounded!(x, H, f, lower, upper, zeros(T, length(f)), tol)
end

function lls_bounded!(
    x::VT, H::AbstractMatrix{T}, f::VT, lower::V, upper::V, g::VT, tol=sqrt(eps(T))
) where {T,VT<:AbstractVector{T},V<:AbstractVector}
    assert_oneindexed(A) = @assert(all(r -> first(r) == 1, axes(A)))
    assert_oneindexed(x)
    assert_oneindexed(H)
    assert_oneindexed(f)
    assert_oneindexed(lower)
    assert_oneindexed(upper)
    assert_oneindexed(g)
    m, n = size(H)
...
function minimum_edges(
    ...
    lower,
    upper,
    minwidth=zeros(eltype(x0), ndims(root));
    extrapolate::Bool=true,
) where {T,N}
    mes = [MELink{T,T}(root) for i in 1:N]
    ...
    for box in leaves(root)
        ...
        for i in 1:N
            ...
            insert!(
                mes[i],
                width(box, i, x0, lower, upper),
                box => extrapolate ? fval + qdelta(box, i) : fval,
            )
        end
    end
    return mes
...
function sweep!(
    ...
    return sweep!(root, mes, f, x0, splits, lower, upper; fvalue=fvalue, minwidth=minwidth)
end
function sweep!(
    root::Box{T},
    mes::Vector{<:MELink},
    ...
    for i in dimorder
        ...
        while !isempty(me)
            ...
            empty!(visited)
            finalbox, minval = autosplit!(
                box, mes, f, x0, xtmp, i, splits, lower, upper, minwidth, visited
            )
            push!(splitboxes, finalbox)
            minval <= fvalueT && return root, unique_smallest_leaves(splitboxes)
        end
    end
    return root, unique_smallest_leaves(splitboxes)
end
...
function analyze!(
    ...
    while boxval > fvalue &&
              (tol_counter <= ndims(root) || !used_quasinewton) &&
              len < maxevals
        lastval = boxval
        numeval0 = numevals(f)
        _, splitboxes = sweep!(
        ...
        if nquasinewton < baseline_evals + numevals(f) < maxevals &&
            boxval > fvalue &&
            !isempty(splitboxes)
            ...
            for (ibox, box) in enumerate(basinboxes)
                ...
                if success_build && all(x -> x > 0, Q.dimpiv)
                    ...
                    @assert isleaf(box)
                    complete_quadratic_model!(
                        Q, c, box, f, x0, xbase, scale, splits, lower, upper, thresh
                    )
                    # If the model is complete, try Quasi-Newton algorithm
                    if Q.nzrows[] == size(Q.coefs, 1) &&
                        minimum(abs.(diag(Q.coefs))) >= thresh
                        g, B = solve(Q)
                        # Bscale = (1./scale) .* B .* (1./scale)'
                        # display(Bscale)
                        # error("stop")
                        leaf, minvalue, α = quasinewton!(
                            box, B, g, c, scale, f, x0, lower, upper
                        )
                        success_qn = α > 0
                        ...
                        if success_qn
                            ...
                            if boxvalue - minvalue <=
                                max(atol, rtol * (abs(boxvalue) + abs(minvalue)))
                            ...
                            if α == 1
                                for j in (ibox + 1):length(basinboxes)
                                    boxj = basinboxes[j]
