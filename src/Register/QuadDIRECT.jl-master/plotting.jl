...
function outerbounds_finite(b1, b2)
    return (min(b1[1], get_finite(b2[1], b1[1])), max(b1[2], get_finite(b2[2], b1[2])))
end
widenbounds(b) = (Δx = 0.1 * (b[2] - b[1]);
return (b[1] - Δx, b[2] + Δx))
...
function plotboxes(
    ...
    lower,
    upper,
    xbounds::Tuple{Any,Any},
    ybounds::Tuple{Any,Any};
    clim=extrema(root),
...
function plotboxes(
    ...
    return plotboxes(
        root, x0, lower, upper, plotbounds(root, x0, lower, upper)...; clim=clim, cs=cs
    )
end
