...
mutable struct RCpair{
    T<:AbstractFloat,N,RType<:AbstractArray{T,N},CType<:AbstractArray{Complex{T},N}
}
    R::RType
    C::CType
    region::Vector{Int}
...
function RCpair{T}(
    ::UndefInitializer, realsize::Dims{N}, region=1:length(realsize)
    ...
    return RCpair(view(R, map(n -> 1:n, realsize)...), C, [region...])
end

function RCpair(A::Array{T}, region=1:ndims(A)) where {T<:AbstractFloat}
    return copy!(RCpair{T}(undef, size(A), region), A)
end
...
function copy(RC::RCpair{T,N}) where {T,N}
    ...
    return RCpair(view(R, RC.R.indices...), C, copy(RC.region))
end

# New API
function rplan_fwd(R, C, region, flags, tlim)
    return FFTW.rFFTWPlan{eltype(R),FFTW.FORWARD,true,ndims(R)}(R, C, region, flags, tlim)
end
function rplan_inv(R, C, region, flags, tlim)
    return FFTW.rFFTWPlan{eltype(R),FFTW.BACKWARD,true,ndims(R)}(R, C, region, flags, tlim)
end
function plan_rfft!(
    RC::RCpair{T}; flags::Integer=FFTW.ESTIMATE, timelimit::Real=FFTW.NO_TIMELIMIT
) where {T}
...
@deprecate RCpair(
    realtype::Type{T}, realsize, region=1:length(realsize)
) where {T<:AbstractFloat} RCpair{T}(undef, realsize, region)
