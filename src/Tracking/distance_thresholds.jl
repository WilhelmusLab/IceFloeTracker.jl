...
"""
    seconds(time_difference::Union{Period,CompoundPeriod})

Convenience function to convert time difference in Millisecond to decimal seconds.

## Arguments
- `time_difference`: Result of subtracting two DateTime instances.
...
be in meters. The final dx value in the list is the maximum displacement. Returns true or false depending
on whether Δx is larger than the expected maximum for the given time period Δt.
...
# TODO: Add fitted parameters for more strict boundaries, e.g. 90, 95th percentiles.
"""
...
"""
    maximum_linear_distance(Δt; umax=0.75, eps=250)

Compute the maximum travel distance based on travel time Δt (a Time Period) based on
the maximum velocity `umax` (m/s) and an additive uncertainty of `eps` (meters).
"""
