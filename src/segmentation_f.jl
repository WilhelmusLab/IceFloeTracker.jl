...
function segmentation_F(
    ...
    leads_opened = ImageMorphology.area_opening(
        leads_filled; min_area=lower_min_area_opening
    )
    ...
    #BW_final4(3)
    leads = convert(BitMatrix, (complement.(leads_bothat) .* leads_opened_branched)) .* .!watershed_intersect
    #BW1
