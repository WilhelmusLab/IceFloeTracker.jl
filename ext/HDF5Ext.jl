...
using IceFloeTracker.Geospatial: latlon
using IceFloeTracker.Segmentation: regionprops_table, converttounits!
using IceFloeTracker.ImageUtils: binarize_mask
import IceFloeTracker.HDF5: load_hdf5, save_hdf5, V1

@info "HDF5Ext.jl loaded."
...
"""
    _choose_dtype(mx::T) where {T<:Integer} -> Type

Choose the smallest integer data type that can represent the value `mx`.
"""
function _choose_dtype(mx::T) where {T<:Integer}
